...
### 0.3.2
- **审批详情气泡（悬浮即办）**：任务气泡里悬停「需要审批」行约 0.15 秒，气泡外侧弹出同风格详情小卡：请求工具名 + 审批理由 + 已等待时长 + 排队笔数，并提供三个操作——「拒绝」「查看详情」「允许一次」（右下角加宽主按钮，半透明浅绿强调；三个按钮等高）。允许/拒绝直接答复 DSH 界面里挂起的那笔审批（走官方 PendingApproval.answer 通道，与在界面里亲手点按钮完全等价，界面卡片同步消失），无需先打开 DSH 窗口；点「查看详情」或点卡片空白区域仍会唤起 DSH 界面并定位到该会话。多笔积压时逐笔放行，放行后卡片自动刷新为下一笔。数据来自状态文件新增的 pendingApprovals 明细（旧壳自动忽略，向后兼容）。
- **提问详情气泡（悬浮作答）**：悬停「需要回答」行弹出粉色同款详情卡，可直接完成作答——单个选择题点选项即提交；多问题批次逐题翻页（‹ ›），答完全部才亮「提交」；多选题勾选后点提交确认；开放题在卡内输入框直接打字（Enter 或提交钮）；计划评审显示「计划评审」徽标与计划正文摘要。走官方 PendingQuestion.answer 通道整批提交，与在 DSH 界面作答完全等价；无法可靠内联作答的批次（问题过多 / 选项过多 / 文本超长）自动降级为只读提示 + 跳转 DSH 界面。数据来自状态文件新增的 pendingQuestions 明细。
- **小窗闪烁根治**：进度窗改为宽限显示（忙碌持续 700ms 才出现，一闪而过的任务不再弹窗）；外部子窗在创建事件时刻即同步隐藏（DshWindow instanceof 判定），不再闪出原生小窗。
- **悬浮球坐标加固**：位置/区域计算收敛到独立模块并全链路钳制，损坏的设置值只回退默认位，不再向窗口 setPosition 送 NaN（修复偶发 setPosition NaN 报错）；slideOrb 对非有限目标直接跳过动画并记 trace。
- **气泡「一键清除」**：气泡右上新增清除按钮（仅当存在带未读白点的已完成任务时显示），一次清掉全部「已完成」任务的白点角标（等效逐行点击、但不跳转会话）；待审批 / 待回答 / 运行中的强提醒不受影响。已读标记持久化到 $DSH_HOME/desktop-shell-read.json（原子写、坏文件自动容错）：同一批完成被重复观测不再复亮，会话之后真正再次完成（finishedAt 晚于清除时刻）仍会正常提醒；点击任务行跳转同样计为已读。
- **链接窗口（微信读书签式）**：target=_blank 链接不再弹 Electron 默认子窗——全部收进一个无边框自绘链接窗：书签标签条与工具条同行排布（2 个以上链接时出现，正好填充中部空白；点切/悬停关/中键关，重复链接自动合并），工具条全图标化：后退 / 前进 / 刷新（加载时旋转）/ 复制链接（成功后打勾反馈）/ 在浏览器中打开 / 最小化 / 最大化 / 关闭；工具条可拖动、双击最大化，无地址栏；每个标签是沙箱 webview（独立分区 persist:dsh-shell-links，不与 dsh 内核共享 cookie，外部页面拿不到任何壳权限），网页里再点 _blank 链接会开成新标签；关闭最后一个标签即关窗。mailto: / tel: 等系统协议直接交系统处理；javascript: / data: 等危险协议一律拒绝。窗口打开全程零闪烁（外部子窗创建即隐藏销毁）。
- **不再有 File/Edit 菜单**：Windows / Linux 彻底移除 Electron 默认应用菜单（主窗 / 设置窗 / 进度窗全无，Alt 也唤不出）；macOS 按系统惯例保留应用菜单。
- **单元测试**：node:test 零依赖单测扩到 83 项（read-store 16 / link-policy 5 / orb-geometry 7 / approval-view 11 / question-view 3 / approval-details 9 / question-details 12 / client 桥接 20），npm test 运行；安装包打包自动排除 *.test.js。

### 0.3.1
- **容器化贴边模型**：悬浮球窗口永远完整落在当前显示器工作区内——半隐效果改为鲸鱼在容器内平移（CSS 位移 + 容器裁切），不再把窗口挂出屏幕边缘。修复双屏内缘停靠时"半只鲸鱼漏到邻屏"与"气泡弹到邻屏"（窗口中心不再骑跨交界线，显示器归属查询恢复稳定）；悬停检测扩大到整个容器，鼠标碰到贴边附近即可唤起。
- **拖拽 DPI 修复**：拖拽改为主进程光标驱动（`getCursorScreenPoint`，与 `setPosition` 同一 DIP 坐标系），渲染层完全不做坐标换算——修复 Windows 非 100% 缩放（125%/150%）及混合多屏 DPI 下拖动不跟手；右键菜单同样按主进程光标定位。
- **跨屏拖拽**：拖拽钳制按"光标所在屏"计算，悬浮球可以拖到任意一块屏，也能拖回来（修复"一旦进 2 屏就再也回不去 1 屏"）。
- **自动贴边可关**：设置窗新增「自动贴边」开关（默认开）。开=原行为（松手半隐+吸附最近边缘）；关=自由模式——落在哪停在哪、鲸鱼完整可见、不自动缩回，气泡优先向右弹（右侧放不下翻左）；自由位置重启后保留。
- **气泡高度实测**：气泡页渲染后实测内容高度回传主进程校正窗高，消除任务列表下方约半行的固定留白（旧固定公式与 CSS 实际尺寸脱节），"+N 个会话"提示行也不再被裁。

### 0.3.0
- **任务悬浮球（三窗口架构）**：小鲸鱼圆盘吸附在屏幕左/右缘半隐待命（圆心压边线、零间隙）；悬停即滑出并展开任务气泡窗（鲸鱼↔气泡视为同一悬停区，350ms 宽限无缝接力），移开自动收起；拖拽跟手，松手回弹吸附最近水平边缘。
- **状态一眼可见**：有任务运行时圆环流光旋转 + 计数角标（≥1 即显示），琥珀=需要审批、粉=需要回答、白=完成未读的关注点；角标与关注点按贴边方向自动翻到露出的半侧；后端断连整体灰化降速。
- **左键**唤起主窗口（顺带收起气泡）；**右键**弹出自定义毛玻璃菜单（刷新窗口 / 启动后端或重启+关闭后端 / 悬浮窗设置），失焦即关、贴边方向自适应、与气泡互斥。
- **气泡窗**：从鲸鱼内侧弹出的毛玻璃卡片（带指向小鲸鱼的尾巴），列出各会话状态（处理中 / 需要审批 / 需要回答 / 已完成+未读白点）；点击任务行直达对应会话（官方 client 面 `ctx.sessions.open`），点击空白仅唤起主窗；任何点击后收起；状态变化自动弹出一次，超时（2–15s 可配）自动收起。
- **设置**：悬浮球设置改为无边框圆角卡片窗、高度随内容自适应；可调圆圈大小（40–96px）、背景透明度、字号、深浅主题、气泡自动收起时长，改动即时生效。
- **官方图标**：悬浮球与设置标题栏改用项目自带黑鲸 LOGO（深色主题自动反白）。
- 破坏性变更：`overlay.pos` 配置改为 `side` + `anchorY`（不兼容 0.2.x 卡片位置/宽度配置）。
...
- **新增「任务悬浮窗」**：桌面置顶的常开小卡片（类 GPU-Z / 硬件监控风格），实时显示 DSH 会话状态——
  ...
  - 头部可拖动（位置自动记忆，托盘可重置），**拖右边缘调整宽度**（220–640px，双击边缘还原默认；高度始终随任务数自动伸缩），背景半透明毛玻璃（20%–100% 透明度、深/浅主题、字号 11–18px 均可调）；
  - **点击任意会话行直达该会话**（经官方 client 面 `ctx.sessions.open()` 切换 Web UI 当前会话，同时把主窗带到前台）；「已完成」行点击后即从悬浮窗删除——悬浮窗只常驻显示处理中/需要审批/需要回答的任务；
  - 「需要审批 / 需要回答 / 已完成」状态出现时整行**闪烁两次**提示；
  - 控制入口：托盘右键 →「任务悬浮窗」子菜单（显示开关 / 设置… / 重置位置和宽度），设置窗改动即时生效。
