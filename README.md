...
### 0.3.0
- **悬浮窗改版为「任务悬浮球」**：实心卡片换成屏幕角落的圆形悬浮球——常态静置鲸鱼图标；有任务运行时圆环流光旋转、右上角显示运行会话数角标（≥1 即显示）。
- **气泡详情**：悬停悬浮球、或出现「需要审批 / 需要回答 / 任务完成」状态变化时，自动展开毛玻璃气泡列出各会话与状态；气泡方向按屏幕剩余空间自动向左/向右弹出，默认 5 秒无操作自动收起（悬停期间不收起）；点击状态行直达对应会话（官方 client 面 `ctx.sessions.open`）。
- **交互**：圆圈可任意拖动（自动贴回工作区）；点击圆圈收起气泡并把主窗口带到前台。
- **新增可调项**：悬浮设置窗加入「圆圈大小」（40–96px）与「气泡自动收起时长」（2–15s）滑条，改动即时生效。
- 移除旧卡片的右缘拖拽调宽交互（气泡宽度自适应内容，位置设置项仅保留「重置位置」）；按约定不兼容 0.2.x 的 overlay.width 配置。
